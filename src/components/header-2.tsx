...
export function Header() {
  ...
  const slowScroll = (target, duration = 1200) => {
        ...
        <div className="hidden items-center gap-2 md:flex">
          {links.map((link, i) => (
            <a
              onClick={link.action}
              key={i}
              className={`${buttonVariants({ variant: "ghost" })} flex items-center justify-center gap-1`}
            >
              {link.icon}
              {link.label}
            </a>
